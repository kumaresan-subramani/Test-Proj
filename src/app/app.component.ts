...
export class AppComponent {
  ...
  public selectMenu(i: number): void {
    this.menuList.forEach((item: Record<string, any>) => { item.isSelect = false; });
    this.menuList[i].isSelect = true;
    document.getElementById(this.menuList[i].menu).scrollIntoView({ behavior: 'smooth' });
  }
  ...
  public selectMobMenu(i: number): void {
    this.menuList.forEach((item: Record<string, any>) => { item.isSelect = false; });
    this.menuList[i].isSelect = true;
    document.getElementById(this.menuList[i].menu + '_mob').scrollIntoView({ behavior: 'smooth' });
  }

  public selectMobileMenu(menu: string): void {
    setTimeout(() => {
      document.getElementById(menu + '_mob').scrollIntoView({ behavior: 'smooth' });
    });
  }
  ...
  public openDialog(type: any): void {
    ...
    const dialogRef = this.dialog.open(ContactComponent, { panelClass: 'custom-dialog-container', data: { type: 'web' } });
    dialogRef.afterClosed().subscribe(result => { console.log(`Dialog result: ${result}`); });
  }

  public openMobileDialog(type: any): void {
    localStorage.setItem('contactType', type);
    const dialogRef = this.dialog.open(ContactComponent, { panelClass: 'custom-dialog-container-mobile', data: { type: 'mobile' } });
    dialogRef.afterClosed().subscribe(result => { console.log(`Dialog result: ${result}`); });
  }
